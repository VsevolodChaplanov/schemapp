...
namespace schemapp::testing {
    ...
    TEST_CASE("indexing by tags in parameters pack tests, check indices is correct") {
        ...
        constexpr auto index_of_bool = schemapp::index_tag<bool_tag, bool_entry, int_entry, double_entry>;
        STATIC_CHECK(index_of_bool == 0);

        constexpr auto index_of_int = schemapp::index_tag<int_tag, bool_entry, int_entry, double_entry>;
        STATIC_CHECK(index_of_int == 1);

        constexpr auto index_of_double = schemapp::index_tag<double_tag, bool_entry, int_entry>;
        STATIC_CHECK(index_of_double >= 2); // check index over
    }

    TEST_CASE("get element using string key using indexing") {
        constexpr auto index_of_bool = schemapp::index_value<"bool_key"_fs, bool_entry, int_entry>;
        STATIC_CHECK(index_of_bool == 0);

        constexpr auto index_of_int = schemapp::index_value<int_entry::tag_value, bool_entry, int_entry>;
        STATIC_CHECK(index_of_int == 1);

        constexpr auto index_of_double = schemapp::index_value<"double_key"_fs, bool_entry, int_entry>;
        STATIC_CHECK(index_of_double == 2); // check index over
    }

    TEST_CASE("indexing by tags in tuple tests, check indices is correct") {
        using bool_entry_found = detail::get_entry_by_tag<struct bool_tag, bool_entry, int_entry, double_entry>;
        STATIC_CHECK(std::is_same_v<bool_entry_found, bool_entry>);

        using int_entry_found = detail::get_entry_by_key<"int_key"_fs, bool_entry, int_entry, double_entry>;
        STATIC_CHECK(std::is_same_v<int_entry_found, int_entry>);

        using double_entry_found = detail::get_entry_by_key<"double_key"_fs, bool_entry, int_entry, double_entry>;
    ...
    TEST_CASE("get entries of tree_test, check correct types") {
        ...
        STATIC_CHECK(
            std::is_same_v<tree_test::get_entry_by_key<"string_tag"_fs>::tag_type, struct string_extra_entry_tag>);
    }
} // namespace schemapp::testing